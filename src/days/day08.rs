...
impl SegmentSet {
    fn parse(s: &str) -> Self {
        SegmentSet(s.bytes().fold(0, |x, i| x | 1 << ((i - b'a') & 7)))
    }
    ...
    fn parse(s: &str) -> Self {
        let vecfrom = |s: &str| {
            s.split_ascii_whitespace()
                .map(SegmentSet::parse)
                .collect::<Vec<_>>()
        };
        let (signal, output) = s.trim().split_once(" | ").unwrap();
        Record {
            signal: vecfrom(signal),
            output: vecfrom(output),
        }
    }
    ...
    fn decode(&self) -> usize {
        ...
        self.output
            ...
            .map(|set| {
                let (len, o1, o4) = (
                    set.len(),
                    set.intersect_length(one),
                    set.intersect_length(four),
                );
                match (len, o1, o4) {
                    (6, 2, 3) => 0,
                    (2, _, _) => 1,
